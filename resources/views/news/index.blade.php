...
<div class="max-w-6xl mx-auto px-4 py-12">
    <h1 class="text-2xl font-bold mb-1">{{ __('public.news') }}</h1>
    <p class="text-slate-500 mb-8">{{ __('public.latest_information') }}</p>
        ...
        <div class="grid md:grid-cols-3 gap-5">
            @foreach ($news as $item)
                <a href="{{ route('news.show', $item) }}" class="block bg-white border rounded-xl overflow-hidden hover:shadow-md transition">
                    @if ($item->thumbnail)
                        <img src="{{ asset('storage/'.$item->thumbnail) }}" alt="{{ $item->localized('title') }}" class="w-full h-40 object-cover">
                    @endif
                    <div class="p-5">
